...
impl DimStyle {
    ...
    /// The values AutoCAD assumes for DIMSTYLE groups that a DXF leaves out.
    ///
    /// Writers omit groups equal to AutoCAD's built-in (imperial) defaults, so
    /// a reader must start from those, not from the metric values [`Self::new`]
    /// uses for freshly created styles. The differences are the ones that
    /// change how a dimension reads: zero suppression, text orientation,
    /// precision, text placement and the line offsets.
