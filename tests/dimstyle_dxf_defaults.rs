//! DIMSTYLE groups a DXF leaves out mean AutoCAD's built-in values.
//!
//! AutoCAD and compatible writers drop DIMSTYLE groups that equal the built-in
//! imperial defaults. Reading those entries from the metric `DimStyle::new`
//! values turned DIMZIN 0 into 8 (so `1.000` became `1`), DIMTIH/DIMTOH on into
//! off (vertical dimensions read sideways) and DIMDEC 4 into 2.
...
/// Groups AutoCAD omits when they hold the built-in value, by DXF code.
...
#[test]
fn omitted_dimstyle_groups_read_as_autocad_defaults() {
    let mut doc = CadDocument::new();
    let mut sparse = DimStyle::new("Sparse");
    // Values that differ from both the metric constructor and AutoCAD's
